(function () {
    ...
    module.exports.list = function (req, res, next) {
        mongoose.model('StreamModel')
            .find({ channel_id: req.params.id })
            .select('id user_id channel_id title group frames fps created updated')
    ...
    module.exports.put = function (req, res, next) {
        var streamObject = req.params;
        mongoose.model('StreamModel')
            .findByIdAndUpdate(req.params.id, streamObject, function (err, data) {
                if (err) {
                    res.send(mongoHandler.handleError(err));
                } else {
                    res.send(200, data);
